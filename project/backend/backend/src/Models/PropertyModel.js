...
propertySchema.pre('save', function (next) {
    // generate slug from propertyName
    this.slug = a5b(this.propertyName, { lower: true });
    // Normalize address fields to aid searching: store as lowercase and without spaces
    if (this.address) {
        if (this.address.city) {
            this.address.city = this.address.city.toString().toLowerCase().replaceAll(' ', '');
        }
        if (this.address.state) {
            this.address.state = this.address.state.toString().toLowerCase().replaceAll(' ', '');
        }
        if (this.address.area) {
            this.address.area = this.address.area.toString().toLowerCase().replaceAll(' ', '');
        }
    }
    next();
});
